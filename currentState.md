# PravahAgent Current State Analysis

## Overview
PravahAgent is a browser automation SDK that uses LLM-powered agents to execute tasks on web pages. It provides both imperative page methods (`page.ai()`, `page.extract()`) and a programmatic task execution API.
...
#### **PravahAgent Class** ([src/agent/index.ts](src/agent/index.ts))

The main class that orchestrates everything:

```typescript
class PravahAgent<T extends BrowserProviders = "Local"> {
  // Core methods
  ...
  async newPage(): Promise<PravahPage>
  async getPages(): Promise<PravahPage[]>
...
#### **PravahPage Interface** ([src/agent/index.ts:567-605](src/agent/index.ts#L567-L605))

Enhanced Playwright `Page` with AI methods:

```typescript
interface PravahPage extends Page {
