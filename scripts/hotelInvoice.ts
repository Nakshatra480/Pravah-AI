...
import { PravahPage } from "../src/types/agent/types";
import { PravahAgent } from "../src/agent";
import { z } from "zod";
...
async function runWorkflow() {
  let agent: PravahAgent | null = null;

  try {
    // Initialize PravahAgent
    console.log("Initializing PravahAgent...");
    agent = new PravahAgent({
      llm: {
        provider: "openai",
        model: "gpt-4o",
      },
    ...
    const page: PravahPage = await agent.newPage();
    if (!page) {
      throw new Error("Failed to get page instance from PravahAgent");
    }
    ...
    // Clean up
    if (agent) {
      console.log("Closing PravahAgent connection.");
      try {
        await agent.closeAgent();
      } catch (err) {
        console.error("Error closing PravahAgent:", err);
      }
    }
  }
}
