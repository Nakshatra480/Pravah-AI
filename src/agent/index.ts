...
import { PravahLLM, createLLMClient } from "@/llm/providers";
import {
  ActionContext,
  ActionType,
...
export class PravahAgent<T extends BrowserProviders = "Local"> {
  ...
  private _variables: Record<string, PravahVariable> = {};
  private errorEmitter: ErrorEmitter;

  public get currentPage(): PravahPage | null {
    if (this._currentPage) {
      return this.setupPravahPage(this._currentPage);
    }
    return null;
  }
  ...
  constructor(params: PravahConfig<T> = {}) {
    ...
    this.browserProvider = (
      this.browserProviderType === "Cloud"
        ? new PravahCloudProvider({
            ...(params.cloudConfig ?? {}),
            debug: params.debug,
          })
        : new LocalBrowserProvider(params.localConfig)
    ) as T extends "Cloud" ? PravahCloudProvider : LocalBrowserProvider;
   ...
   */
  public getVariables(): Record<string, PravahVariable> {
   ...
   */
  public addVariable(variable: PravahVariable): void {
   ...
   */
  public getVariable(key: string): PravahVariable | undefined {
   ...
   * @returns Array of PravahPage objects
   */
  public async getPages(): Promise<PravahPage[]> {
    if (!this.browser) {
      await this.initBrowser();
    }
    if (!this.context) {
      throw new PravahError("No context found");
    }
    return this.context.pages().map(this.setupPravahPage.bind(this), this);
  }

  /**
   * Create a new page in the context
   * @returns PravahPage object
   */
  public async newPage(): Promise<PravahPage> {
    if (!this.browser) {
      await this.initBrowser();
    }
    if (!this.context) {
    ...
    return this.setupPravahPage(page);
  }
  ...
  public async closeAgent(): Promise<void> {
    await disposeAllCDPClients().catch((error) => {
      console.warn("[PravahAgent] Failed to dispose CDP clients:", error);
    });
  ...
  public async getCurrentPage(): Promise<Page> {
    ...
    if (pages.length > 0) {
      ...
      if (lastPage && !lastPage.isClosed() && lastPage !== this._currentPage) {
        if (this.debug) {
          console.log(
            `[PravahAgent] Polling detected new page, switching focus: ${lastPage.url()}`
          );
        }
  ...
  private getTaskControl(taskId: string): Task {
    ...
    if (!taskState) {
      throw new PravahError(`Task ${taskId} not found`);
    }
    return {
      id: taskId,
    ...
    const onPage = async (newPage: Page) => {
      try {
        ...
        if (opener === activeTaskPage) {
          if (this.debug) {
            console.log(
              `[PravahAgent] Task following new tab: ${newPage.url()}`
            );
          }
  ...
  public async runFromActionCache(
    ...
    /**
     * Type-safe dispatch for PravahPage perform* methods.
    ...
    for (const step of [...cache.steps].sort(
      ...
      const page = getPage();
      const pravahPage = page as PravahPage;
      ...
      if (step.actionType === "goToUrl") {
        ...
        if (!url || typeof url !== "string") {
          ...
          await pravahPage.goto(url, { waitUntil: "domcontentloaded" });
          await waitForSettledDOM(pravahPage);
          markDomSnapshotDirty(pravahPage);
          result = {
            taskId: cache.taskId,
        ...
        await pravahPage.reload({ waitUntil: "domcontentloaded" });
        await waitForSettledDOM(pravahPage);
        markDomSnapshotDirty(pravahPage);
        result = {
          taskId: cache.taskId,
        ...
        await pravahPage.waitForTimeout(waitMs);
        result = {
          taskId: cache.taskId,
          status: TaskStatus.COMPLETED,
          steps: [],
        ...
        try {
          ...
          const extractResult = await pravahPage.extract(step.instruction);
          result = {
            taskId: cache.taskId,
            status: TaskStatus.COMPLETED,
            steps: [],
        ...
        if (method && validHelperMethods.has(method)) {
          ...
          result = await pravahPage.perform(step.instruction);
        } else {
          result = {
            taskId: cache.taskId,
            status: TaskStatus.FAILED,
    ...
    try {
      ...
      if (params.success && params.element) {
        ...
        const availableElements = this.collectInteractiveElements(
          params.elementMap,
          PravahAgent.AIACTION_CONFIG.MAX_DEBUG_ELEMENTS_TO_STORE
        );
  ...
  public async executeSingleAction(
    ...
    try {
      ...
      const {
        ...
        instruction,
        initialPage,
        PravahAgent.AIACTION_CONFIG.MAX_RETRIES,
        PravahAgent.AIACTION_CONFIG.RETRY_DELAY_MS,
        startTime
      );
      ...
      // Re-throw PravahErrors as-is
      if (error instanceof PravahError) {
        throw error;
      }
      // Wrap other errors
      const errorMsg = error instanceof Error ? error.message : String(error);
      throw new PravahError(`Failed to execute action: ${errorMsg}`, 500);
    }
  }
  ...
  private setupPravahPage(page: Page): PravahPage {
    const pravahPage = page as PravahPage;

    // Clean up existing listener if this page was already setup
    if ((pravahPage as any)._scopeListenerCleanup) {
      (pravahPage as any)._scopeListenerCleanup();
    }
    ...
    const handleClose = (p: Page) => {
      ...
      if (idx !== -1) {
        if (this.debug) {
          console.log(`[PravahPage] Tab closed, removing from stack`);
        }
    ...
    const onPage = async (newPage: Page) => {
      try {
        ...
        if (opener === getActivePage()) {
          if (this.debug) {
            console.log(
              `[PravahPage] Auto-switching to new tab (Push): ${newPage.url()}`
            );
          }
      ...
      for (let i = 0; i < maxRetries; i++) {
        try {
          ...
          if (
            ...
            if (this.debug) {
              console.log(
                "[PravahPage] Action aborted due to tab switch, retrying on new page..."
              );
            }
    ...
    pravahPage.ai = (task: string, params?: TaskParams) =>
      this.executeTask(task, params, getActivePage());

    pravahPage.perform = (instruction: string, params?: TaskParams) =>
      executeSingleActionWithRetry(instruction, params);

    pravahPage.aiAction = async (instruction: string, params?: TaskParams) => {
      return executeSingleActionWithRetry(instruction, params);
    };

    pravahPage.getActionCache = (taskId: string) => this.getActionCache(taskId);

    pravahPage.runFromActionCache = (cache, params) =>
    ...
    attachCachedActionHelpers(deps, pravahPage);
    ...
    pravahPage.aiAsync = (task: string, params?: TaskParams) =>
      this.executeTaskAsync(task, params, getActivePage());

    pravahPage.extract = async (task, outputSchema, params) => {
