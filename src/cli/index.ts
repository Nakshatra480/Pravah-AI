...
import { PravahError } from "@/agent/error";
...
program
  .name("pravah")
  .description("Pravah AI — Autonomous Browser Agent powered by OpenRouter")
  .version("1.0.2");
...
program
  ...
  .option("--cloud", "Use the Pravah Cloud browser provider")
  .action(async function () {
    const options = this.opts();
    const debug = (options.debug as boolean) || false;
    const useHB = (options.cloud as boolean) || false;
    ...
    console.log(chalk.magenta("🌊 Pravah AI — Browser Agent"));
    currentSpinner.info(
      `Pause using ${chalk.bold("ctrl + p")} and resume using ${chalk.bold("ctrl + r")}\n`
    );
    try {
      // Check for API key if using Pravah Cloud
      if (useHB && !process.env.PRAVAH_CLOUD_API_KEY) {
        const apiKey = await inquirer.password({
          message:
            "Pravah Cloud API key not found in environment variables. Please enter it here:",
          mask: "*",
        });
        if (!apiKey) {
          console.log(
            chalk.yellow("Pravah Cloud API key is required. Exiting.")
          );
          process.exit(0);
        }
        process.env.PRAVAH_CLOUD_API_KEY = apiKey; // Set it for the current process
      }

      const agent = new PravahAgent({
        debug: debug,
        browserProvider: useHB ? "Cloud" : "Local",
        customActions: [
      ...
      process.stdin.on("keypress", async (ch, key) => {
        if (key && key.ctrl && key.name == "p") {
          ...
          currentSpinner.start(
            chalk.blue(
              "Pravah will pause after completing this operation. Press Ctrl+r again to resume."
            )
          );
          currentSpinner.stopAndPersist({ symbol: "⏸" });
          currentSpinner = ora();
          ...
          if (task.getStatus() == TaskStatus.PAUSED) {
            currentSpinner.start(chalk.blue("Pravah will resume"));
            currentSpinner.stopAndPersist({ symbol: "⏵" });
            currentSpinner = ora();
      ...
      const onComplete = async (params: TaskOutput) => {
        console.log(
          boxen(params.output || "No Response", {
            title: chalk.yellow("Pravah AI Response"),
        ...
        if (continueTask) {
          const taskDescription = await inquirer.input({
            message: "What should PravahAgent do next for you?",
            required: true,
          });
      ...
      if (!taskDescription) {
        if (filePath) {
          ...
          taskDescription = await inquirer.input({
            message: "What should PravahAgent do for you today?",
            required: true,
          });
        }
      }
      ...
      if (useHB && !debug) {
        ...
        console.log(`Pravah Cloud Live URL: ${session.liveUrl}\n`);
      }
      ...
      if (err instanceof PravahError || err instanceof Error) {
