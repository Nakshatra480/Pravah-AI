 ...
 */
window.__pravah_collectBoundingBoxes = function(backendNodeIds) {
  const results = {};
 ...
 * @returns Object mapping backend node ID to bounding box
 */
window.__pravah_collectBoundingBoxesByXPath = function(xpathToBackendId) {
  const boundingBoxes = {};
 ...
 * @returns Object mapping backend node ID to bounding box
 */
window.__pravah_collectBoundingBoxesForSameOriginIframe = function(elementsData) {
  const boundingBoxes = {};
...
async function batchCollectBoundingBoxesViaCDP(
  ...
  try {
    ...
    const response = await session.send<{
      ...
      functionDeclaration:
        "function(xpathMappingJson) { try { const data = JSON.parse(xpathMappingJson); return (window.__pravah_collectBoundingBoxesByXPath && window.__pravah_collectBoundingBoxesByXPath(data)) || {}; } catch (error) { return {}; } }",
