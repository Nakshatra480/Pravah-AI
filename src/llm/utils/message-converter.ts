import { PravahMessage, PravahContentPart } from "../types";
import type {
  MessageParam,
  ContentBlockParam,
  ImageBlockParam,
...
export function convertToOpenAIMessages(messages: PravahMessage[]) {
  return messages.map((msg) => {
    ...
    if (typeof msg.content === "string") {
      ...
      openAIMessage.content = msg.content.map((part: PravahContentPart) => {
        if (part.type === "text") {
...
export function convertToAnthropicMessages(messages: PravahMessage[]) {
...
export function convertToGeminiMessages(messages: PravahMessage[]) {
  ...
  for (const msg of messages) {
    ...
    if (typeof msg.content === "string") {
      ...
      geminiMessage.parts = msg.content.map((part: PravahContentPart) => {
        if (part.type === "text") {
