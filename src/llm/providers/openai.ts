...
const ENV_STRUCTURED_SCHEMA_DEBUG =
  process.env.PRAVAH_DEBUG_STRUCTURED_SCHEMA === "1" ||
  process.env.PRAVAH_DEBUG_STRUCTURED_SCHEMA === "true";
...
export interface OpenAIClientConfig {
  ...
  maxTokens?: number;
  baseURL?: string;
  /** Per-request timeout in ms. Omitted = OpenAI SDK default (10 minutes). */
  timeout?: number;
  /** SDK-level retries for connection errors. Omitted = SDK default. */
  maxRetries?: number;
}

/**
 * Convert OpenAI's content format back to PravahContentPart format
 */
function convertFromOpenAIContent(
  content: any
): string | PravahContentPart[] {
...
export class OpenAIClient implements PravahLLM {
