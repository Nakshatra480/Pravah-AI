...
page.ai("Find a flight from NYC to LAX")
  ↓
PravahAgent.executeTask(task, params, page)
  ↓
...
page.aiAction("click the login button")
  ↓
PravahAgent.executeSingleAction(instruction, page, params)
  ↓
...
const result = await session.send("Runtime.evaluate", {
  expression: `window.__pravah_collectBoundingBoxesByXPath(${JSON.stringify(xpathToBackendIdMap)})`,
...
└───────────────┬────────────────────────┘
    ...
    1. ensureScriptInjected(session, contextId)
       └─> Inject window.__pravah_collectBoundingBoxesByXPath
