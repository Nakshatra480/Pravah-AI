...
PravahAgent (`@pravah/agent`) is a TypeScript browser automation SDK that augments Playwright with LLM-powered agents. It exposes three core APIs on `PravahPage`: `page.ai()` for multi-step tasks with screenshots, `page.perform()` for single granular actions (text-only, no screenshots), and `page.extract()` for structured data extraction with Zod schemas. Supports OpenAI, Anthropic, Gemini, and DeepSeek as LLM providers, plus local and cloud (Hyperbrowser) browser backends.
...
- `src/index.ts` — Package entry, exports `PravahAgent` and `TaskStatus`
- `src/agent/index.ts` — `PravahAgent` class: orchestrates browser sessions, LLM providers, MCP clients, custom actions, and task execution
...
| `browser-providers/` | `LocalBrowserProvider` (Playwright chromium with stealth flags) and `PravahCloudProvider` (cloud sessions). Extend base class in `types/browser-providers/types.ts` |
| `llm/` | LLM provider abstraction. `createLLMClient()` factory in `providers/index.ts`. Each adapter implements `PravahLLM` interface |
| `types/` | Centralized type definitions. Define interfaces here before wiring features elsewhere |
| `custom-actions/` | Extension point for domain-specific actions via `PravahConfig.customActions` |
...
`PravahPage.getActionCache()` records XPath, frame index, method, and arguments for deterministic replay via `runFromActionCache()`. Implementation in `agent/shared/action-cache*`.

## Path Aliases

Configured in `tsconfig.json`:
- `@/*` → `./src/*` (use this for internal imports instead of relative paths)
- `@pravah/agent` → `./src/index`
...
| `PRAVAH_CLOUD_API_KEY` | Cloud browser provider |
| `PRAVAH_TRACE_WAIT` | Enable DOM settle tracing (`"1"`) |
| `PRAVAH_PROFILE_DOM` | Enable DOM capture profiling (`"1"`) |
| `PRAVAH_DEBUG_STRUCTURED_SCHEMA` | Log structured LLM schemas (`"1"`) |
